...
class EllipticFourierDescriptors:
    ...
    def continuous_components(self, delta_X, delta_Y, delta_t, t, T, K):
    ...
    def rotation_and_scale_invariance(self, efds):    
    ...
    def get_elliptic_fourier_descriptors_segment(self, delta_X, delta_Y, N):
        ...
        #A0, C0
        efds[0,0], efds[0,2] = self.continuous_components(delta_X = delta_X, delta_Y = delta_Y,
                                                    delta_t = delta_t, t = t, T = T, K = K )
            
        efds = self.rotation_and_scale_invariance(efds)
    ...
    def elliptic_fourier_descriptors(self, bin_im,N):
        
        deltaXY_list = self.deltaX_deltaY(bin_im)
        ...
        for deltaX, deltaY in deltaXY_list:
            efd, K, T = self.get_elliptic_fourier_descriptors_segment(deltaX,deltaY,N)
            efds[counter,:,:] = efd
            K_array[counter] = K
            T_array[counter] = T
            counter = counter + 1
